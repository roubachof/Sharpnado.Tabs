...
public partial class TabHostView : ContentView
{
    ...
    private void OnTabsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
    {
        switch (e.Action)
        {
            ...
            case NotifyCollectionChangedAction.Remove:
                foreach (object tab in e.OldItems)
                {
        ...
        if (UseMauiTapGesture || DeviceInfo.Platform == DevicePlatform.WinUI)
        {
            tabItem.GestureRecognizers.Remove(
                tabItem.GestureRecognizers.FirstOrDefault(
                    g => g is TapGestureRecognizer tapGestureRecognizer
                        && ReferenceEquals(tapGestureRecognizer.Command, TabItemTappedCommand)));
        }
        else
        {
            TouchEffect.SetColor(tabItem, Colors.Transparent);
            Commands.SetTap(tabItem, null);
            Commands.SetTapParameter(tabItem, null);
        }
    }
    ...
    private void RegisterEffects(TabItem tabItem)
    {
        ...
        {
            TouchEffect.SetColor(tabItem, tabItem.SelectedTabColor);
            Commands.SetTap(tabItem, TabItemTappedCommand);
            Commands.SetTapParameter(tabItem, tabItem);
        }
    }
