...
public record SillyDudeTab(string Name, bool IsSelectable);
...
public class SillyDudeViewModel : ANavigableViewModel
{
    ...
    public ObservableCollection<SillyDudeTab> TabTitles { get; private set; }
    ...
    private async Task TestTabsItemsSourceNotifications()
    {
        while (true)
        {
            ...
            if (remove)
            {
                string name = TabTitles[index].Name;
            ...
            else
            {
                ...
                TabTitles.Insert(index, new SillyDudeTab(name, index % 3 != 0));
            }
        }
    }
}